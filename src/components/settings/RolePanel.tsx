...
import { ChipStyle, RoleRule, RoleRuleSet, RuleAction } from "../../api/rules"
...
export interface RolePanelProps {
    role_rule_sets: state<RoleRuleSet[]>;
}

const RoleRuleItem = React.memo((props: {
    rule: RoleRule,
    index: number,
    set_index: number,
    onRuleChange: (set_index: number, index: number, rule: RoleRule) => void
}) => {
    ...
    const update_rule = (rule: RoleRule) => {
...
const RolePanel = (props: RolePanelProps) => {
    return <RuleSetPanel
        ...
        exclusive
        rule_sets={props.role_rule_sets}
        ruleItemElement={RoleRuleItem}
    />
}
